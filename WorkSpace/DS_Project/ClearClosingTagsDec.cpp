...
bool ClearClosingTagsDec::needClosingTag(std::string& tag) const
{
    ...
    std::string subTag = tag.substr(1, tag.length() - 2); // Extract the substring
    bool isChild = tagsStack->top()->isChild(&subTag);
	if (isChild) {
        tagsStack->push(tagsStack->top()->getChild(&subTag));
		return false;
	}
	else {
		return true;
